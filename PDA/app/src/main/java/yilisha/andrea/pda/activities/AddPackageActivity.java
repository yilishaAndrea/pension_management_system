...
public class AddPackageActivity extends AppCompatActivity {
    ...
    private Handler handler = new Handler() {
        public void handleMessage(android.os.Message msg) {
            if(msg.what == NetConnect.CHANGE_UI) {
                Toast.makeText(AddPackageActivity.this, "加包成功", Toast.LENGTH_LONG).show();
            }else if(msg.what ==NetConnect.ERROR) {
                Toast.makeText(AddPackageActivity.this, "操作失败，请重新输入", Toast.LENGTH_LONG).show();
            }
        };
    };
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addPackageBT = (Button)findViewById(R.id.addPackageBT);

        addPackageBT.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                pickNumberStr = pickNumberET.getText().toString();
                packageNumberStr = packageNumberET.getText().toString();
                if(pickNumberStr.length()!=6||packageNumberStr.length()!=10){
                    Toast.makeText(AddPackageActivity.this,"请输入6位面单号和10位集包号！",Toast.LENGTH_LONG).show();
                }else{
                    //发送请求到服务器
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            String url = new NetConnect().ADD_PACKAGE_URL+"?bigBagNo=" + packageNumberStr + "&goodNo=" + pickNumberStr;
                            addPackage(url);
                        }
                    }).start();
                }
    ...
    //登录往服务器发送请求
    private int addPackage(String urlPath){
        int id=0;
        try {
